...
class OrdemForm extends TPage
{
    ...
    function __construct()
    {
        ...
        $status->addItems( [ 'aberta'=>'Aberta', 'produzindo'=>'Produzindo','fechada'=>'Fechada'] );
        $data_inicio->setMask('dd/mm/yyyy hh:ii');
        $data_inicio->setDatabaseMask('yyyy-mm-dd hh:ii');
        $status->setDefaultOption('Aberta');

        // add the fields
        $this->form->addQuickField('Id',$id,20);
        $this->form->addQuickField('Numero', $numero,  100 , new TRequiredValidator);
        $this->form->addQuickField('Equipamento', $equipamento_id,  100 , new TRequiredValidator);
        $this->form->addQuickField('Produto', $produto_id,  100 , new TRequiredValidator);
